...
function createPost($request_values)
{
	...
	if (count($errors) == 0) {
		...
		$post_check_query = "SELECT id FROM posts WHERE slug='$post_slug' LIMIT 1";
		$result = mysqli_query($conn, $post_check_query);

		$post = mysqli_fetch_assoc($result);
		$post_id = $post['id']; 
		$topic_id = $request_values['topic_id'];
		$query2 = "INSERT INTO post_topic (post_id, topic_id) VALUES ($post_id, $topic_id)";
...
function updatePost($request_values)
{
	...
	// if (count($errors) == 0) {
		...
		$post_id = $post['id']; 
		$topic_id = $request_values['topic_id'];
		$query2 = "UPDATE post_topic SET topic_id = $topic_id WHERE post_id = $post_id";
		mysqli_query($conn, $query2);
